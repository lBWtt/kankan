...
def create_token_pair(user_id: uuid.UUID) -> dict:
    ...
    Redis 写失败时转成 503 DEPENDENCY_DOWN：避免抛裸 RedisError 导致 500，
    也避免令牌已签发但白名单未登记的"幽灵账号"（refresh 令牌永远换不了新）。
    auth.py 的 login 调用方负责在 503 时回滚用户落库，本函数只保证令牌与白名单原子一致。
    """
    ...
    try:
        redis_client.setex(
            f"{REFRESH_ALLOWLIST_PREFIX}{refresh_jti}",
            settings.jwt_refresh_ttl_seconds,
            str(user_id),
        )
    except RedisError as exc:
        logger.error("写入 refresh 白名单失败：%s", exc)
...
def rotate_refresh_token(token: str) -> uuid.UUID:
    ...
    Redis delete 失败同样转 503：delete 返回 0 与真失败的语义不同——
    返回 0 是"令牌已失效"（401），异常是"Redis 挂了"（503，应让客户端重试而非强制重登）。
    """
    ...
    try:
        deleted = redis_client.delete(f"{REFRESH_ALLOWLIST_PREFIX}{jti}") if jti else 0
    except RedisError as exc:
        logger.error("删除 refresh 白名单失败：%s", exc)
        raise AppError(503, "DEPENDENCY_DOWN", "认证服务暂不可用，请稍后重试")
    if deleted != 1:
